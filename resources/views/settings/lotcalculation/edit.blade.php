...
<div class="card add-new-location mt-2">
    <div class="card-body">
        <form action="{{ route('settings.papersettings.update-paper-quantity', ['id' => encrypt($quantity->id)]) }}" method="POST" id="paper-quantity-edit-form">
            @csrf
            <div class="row">
                <div class="col-md-12">
                    @include('utils.alert')
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select" aria-label="Default select example" id="measurement_type_unit" name="measurement_type_unit">
                            ...
                            @foreach ($unitMeasure as $value)
                            <option value="{{ $value->id }}" {{ $value->id == $quantity->measurement_type_unit ? 'selected' : null }}>{{
                                $value->measurement_unuit }}</option>
                            @endforeach
                            @endif
                        </select>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="no_of_sheet" id="no_of_sheet" value="{{ $quantity->no_of_sheet }}"/>
                        <small class="text-danger error_no_of_sheet"></small>
                    </div>
                </div>
            </div>
            <div class="text-end">
                <button type="reset" class="btn grey-primary reset_edit_paper_type">Cancel</button>
...
<script>
  $(document).ready(function() {
    ...
    $(document).on('click','.reset_edit_paper_type',function(e){
      $('#paper-quantity-edit-form')[0].reset();
    });    
});
</script>
@endsection